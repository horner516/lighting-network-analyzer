...
type Port = { index: number; label: string; direction: string; inputAddress: number | null; outputAddress: number | null; inputProtocol: string | null; outputProtocol: string | null; active: boolean | null; rdm: boolean | null; error: string | null };
type NodeInfo = { ip: string; checkedAt: number; responding: boolean; name: string; description: string; source: string; report: string; subnetMask: string | null; firmwareCode: number | null; mac: string; proplex: boolean; ports: Port[]; note: string; error: string };
...
export function DeviceCards({ devices, query }: { devices: ManualDevice[]; query: string }) {
  ...
  return <div className="space-y-4">
    ...
    {visible.map(device => {
      ...
      const columns = ports.length === 6 ? 3 : ports.length === 8 ? 8 : Math.min(8, Math.ceil(ports.length / 2));
      ...
      return <section key={device.ip} aria-label={name} className="overflow-hidden rounded-xl border border-white/15 bg-[#171d22]">
        ...
        {ports.length > 0 && <><div className="border-t border-white/10 px-4 py-2 text-sm text-slate-400">{ports.length} reported ports · select a port for details</div><div className="overflow-x-auto px-4 pb-4"><div className="grid gap-2" style={{ gridTemplateColumns: `repeat(${columns}, minmax(100px, 1fr))`, minWidth: columns * 108 }}>
          {ports.map(port => {
            const address = port.outputAddress ?? port.inputAddress;
            const protocol = port.outputProtocol || port.inputProtocol;
            const color = port.error ? '#ff5b55' : port.active ? '#28ed20' : '#84948b';
            return <Dialog key={port.index}><DialogTrigger className="rounded-lg border border-[#858b88] bg-[#0d0f0e] px-2 py-1 text-left text-white hover:border-teal-200 focus-visible:outline-2 focus-visible:outline-teal-200" aria-label={`Port ${port.label}, ${port.direction}, ${address === null ? 'universe unknown' : `address ${address}`}`}><div className="flex items-center gap-2 border-b-2 pb-1 font-mono text-sm" style={{ borderColor: color }}><span>{port.label}</span><span>{port.direction === 'Unknown' ? '—' : port.direction}</span></div><div className="mt-1 text-center font-mono text-3xl font-semibold leading-none">{address ?? '—'}</div><div className="mt-1 text-center text-sm" style={{ color }}>{protocol || 'Unknown'}</div><div className="text-center text-xs text-slate-400">RDM {port.rdm === null ? 'unknown' : port.rdm ? 'on' : 'off'}</div></DialogTrigger><DialogContent className="border-white/15 bg-[#171d22] text-slate-100"><DialogHeader><DialogTitle>{name} · Port {port.label}</DialogTitle><DialogDescription className="text-slate-400">Reported by the node at the last poll. Port controls here are read-only.</DialogDescription></DialogHeader><dl className="grid grid-cols-2 gap-4 text-sm">{[['Direction',port.direction],['Input address',port.inputAddress ?? 'Not reported'],['Output address',port.outputAddress ?? 'Not reported'],['Protocol',protocol || 'Not reported'],['RDM',port.rdm === null ? 'Not reported' : port.rdm ? 'On' : 'Off'],['Data at last poll',port.active === null ? 'Not reported' : port.active ? 'Present' : 'Not reported active']].map(([label,value]) => <div key={label}><dt className="text-slate-400">{label}</dt><dd className="mt-1">{value}</dd></div>)}</dl>{port.error && <p className="text-amber-200">{port.error}</p>}<p className="text-sm text-slate-400">{node?.note}</p></DialogContent></Dialog>;
          })}
        </div></div></>}
        <div className="space-y-2 border-t border-white/10 px-4 py-3 text-sm text-slate-400">
          ...
          {node?.responding && <><p className="font-mono text-slate-200">{node.report || 'No status text reported'}</p><p>MAC {node.mac || 'Not reported'} · Firmware code {node.firmwareCode ?? 'Not reported'}</p><p>{node.note}</p></>}
          {node && <p>Last poll: {new Date(node.checkedAt).toLocaleTimeString()} · Device health is not continuously monitored.</p>}
          <a href={`http://${device.ip}/device.htm`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-teal-200 underline">Open device web page <ExternalLink size={14}/></a>
        </div>
      </section>;
    })}
  </div>;
