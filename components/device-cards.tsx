...
export type NodeInfo = { ip: string; checkedAt: number; responding: boolean; online?: boolean; reachabilitySource?: 'web' | 'ping' | 'none'; name: string; description: string; source: string; report: string; subnetMask: string | null; firmwareCode: number | null; firmware?: string; uptime?: string; mac: string; proplex: boolean; universeEditing?: 'proplex-web'; ports: Port[]; note: string; error: string; sessionName?: string | null; showFile?: string | null; sessionStatus?: string | null; metadataStatus?: string };
...
function PortFace({ port, editing, value, onChange }: { port: Port; editing: boolean; value: string; onChange: (value: string) => void }) {
  ...
  return <Dialog><DialogTrigger className="rounded-lg border border-[#77877f] bg-[#0d100f] px-2 py-1 text-left text-white hover:border-teal-200 focus-visible:outline-2 focus-visible:outline-teal-200" style={{ boxShadow: appearance.shadow }} title={appearance.liveOutput ? 'Output activity reported by device' : 'Protocol color · activity not confirmed'} aria-label={`Port ${port.label}, ${port.direction}, ${address === null ? 'universe unknown' : `universe ${address}`}`}>{content}</DialogTrigger><DialogContent className="border-white/15 bg-[#171d22] text-slate-100"><DialogHeader><DialogTitle>Port {port.label}</DialogTitle><DialogDescription className="text-slate-400">Reported by the node at the last poll.</DialogDescription></DialogHeader><dl className="grid grid-cols-2 gap-4 text-sm">{[['Direction',port.direction],['Input universe',port.inputAddress ?? 'Not reported'],['Output universe',port.outputAddress ?? 'Not reported'],['Protocol',protocol || 'Not reported'],...(port.frameRate !== undefined ? [['Configured frame rate', port.frameRate === null ? 'Not reported' : `${port.frameRate} Hz`], ['Merge', port.mergeMode || 'Not reported'], ['Channel range', `${port.channelFrom ?? '—'}–${port.channelTo ?? '—'}`]] : []),['RDM',port.rdm === null ? 'Not reported' : port.rdm ? 'On' : 'Off'],['Data at last poll',port.active === null ? 'Not reported' : port.active ? 'Present' : 'Not reported active']].map(([label,item]) => <div key={label}><dt className="text-slate-400">{label}</dt><dd className="mt-1">{item}</dd></div>)}</dl>{port.addressNote && <p className="text-sm text-slate-400">{port.addressNote}</p>}{port.error && <p className="text-amber-200">{port.error}</p>}</DialogContent></Dialog>;
}

function DeviceCard({ device, node, polling, deviceType, onEditUniverses }: { device: ManualDevice; node?: NodeInfo; polling: boolean; deviceType: ManualDevice['deviceType']; onEditUniverses?: (ip: string, updates: UniverseUpdate[]) => Promise<void> }) {
...
export function DeviceCards({ devices, query, info, pollingIp, deviceType = 'Node', pollBusy = false, onPoll, onEditUniverses }: { devices: ManualDevice[]; query: string; info: Record<string, NodeInfo>; pollingIp: string; deviceType?: ManualDevice['deviceType']; pollBusy?: boolean; onPoll?: () => Promise<void>; onEditUniverses?: (ip: string, updates: UniverseUpdate[]) => Promise<void> }) {
