...
type State = { available: boolean; enabled: boolean; protocol: string; universe: number; channel: number; value: number; currentValue: number; allAtFifty: boolean; frameRate: number; error?: string };
...
export function SignalTransmitter() {
  ...
  useEffect(() => {
    ...
    fetch('/api/transmitter', { cache: 'no-store', signal: controller.signal }).then(async response => {
      ...
      if (!state.available) throw Error(); setRemote(state); setProtocol(state.protocol); setUniverse(String(state.universe)); setChannel(String(state.channel)); setValue(String(state.currentValue));
  ...
  async function update(extra: Record<string, unknown>, success: string) {
    ...
    try {
      const response = await fetch('/api/transmitter', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ protocol, universe: Number(universe), channel: Number(channel), value: Number(value), ...extra }) });
      ...
      setRemote(result); setProtocol(result.protocol); setUniverse(String(result.universe)); setChannel(String(result.channel)); setValue(String(result.currentValue)); setMessage(success);
  ...
  return <section aria-label="DMX transmitter" className="overflow-hidden rounded-lg border border-white/10 bg-[#171d22]">
    ...
    <form onSubmit={event => { event.preventDefault(); void update({ setChannel: true }, `Channel ${channel} set to ${value}.`); }} className="space-y-5 p-4">
      ...
      <div className="text-sm text-slate-400"><p>{enabled ? `${remote?.protocol} universe ${remote?.universe} · ${remote?.frameRate} packets/s · output active` : 'Output is off. Configure values before enabling if desired.'}</p>{message && <p role="status" className="mt-2 text-amber-200">{message}</p>}</div>
    </form>
  </section>;
}
