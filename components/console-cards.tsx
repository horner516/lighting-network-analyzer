    ...
    {visible.map(device => {
      ...
      return <section key={device.ip} className="overflow-hidden rounded-xl border border-white/15 bg-[#171d22]">
        ...
        <div className="grid gap-3 border-t border-white/10 p-4 md:grid-cols-3">
          <div className="rounded-lg bg-black/15 p-4"><div className="text-sm text-slate-400">{isMa ? 'Session' : 'Console'}</div><div className="mt-2 text-lg">{isMa ? node?.sessionName || 'Not reported' : node?.consoleBrand || 'Detecting brand'}</div>{isMa && node?.showFile && <p className="mt-2 text-sm text-slate-300">Show file: {node.showFile}</p>}{isMa && node?.sessionStatus && <p className="mt-1 text-xs text-slate-400">{node.sessionStatus}</p>}{isEtc && <p className="mt-2 text-sm text-slate-300">{node?.consoleFamily} · OSC TCP {node?.consoleServicePort}</p>}{(!isMa || !node?.sessionName) && <p className="mt-2 text-xs text-slate-500">{node?.metadataStatus || 'Waiting for console brand detection.'}</p>}</div>
