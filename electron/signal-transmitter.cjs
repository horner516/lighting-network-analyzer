...
function validated(input = {}) {
  ...
  return { protocol, universe, channel, value };
}
...
function sacnFrame(universe, levels, sequence, cid) {
  ...
  packet.writeUInt16BE(0x7258, 38); packet.writeUInt32BE(2, 40); packet.write('Lux Link Transmitter', 44, 'utf8'); packet[108] = 100; packet[111] = sequence; packet.writeUInt16BE(universe, 113);
...
function createSignalTransmitter({ createSocket = type => dgram.createSocket(type), intervalMs = 33, cid = crypto.randomBytes(16) } = {}) {
  const socket = createSocket('udp4'); socket.bind(0, () => socket.setBroadcast(true));
  const levels = Buffer.alloc(512); let enabled = false, sequence = 0, timer = null;
  let config = { protocol: 'sACN', universe: 1, channel: 1, value: 0 };
  function send() {
    if (!enabled) return;
    sequence = sequence >= 255 ? 1 : sequence + 1;
    const packet = config.protocol === 'sACN' ? sacnFrame(config.universe, levels, sequence, cid) : artNetFrame(config.universe, levels, sequence);
  ...
  function snapshot() { return { available: true, enabled, ...config, currentValue: levels[config.channel - 1], allAtFifty: levels.every(value => value === 128), frameRate: Math.round(1000 / intervalMs) }; }
