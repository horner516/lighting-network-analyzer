...
export function portAppearance(port: PortAppearanceInput) {
  ...
  const noData = port.direction === 'OUT' && port.active === false && !fault;
  ...
  const text = fault ? '#ff7068' : noData ? '#ff2f28' : !configured ? '#84948b' : protocol === 'sACN' ? '#41ec3c' : protocol === 'Art-Net' ? '#8bc6ff' : protocol === 'Art-Net / sACN' ? '#64dfd1' : '#84948b';
  ...
  return { line, text, value: fault ? '#ff7068' : noData ? '#ff2f28' : '#ffffff', noData, liveOutput, shadow: liveOutput ? `0 0 9px ${line}55` : 'none' };
}
