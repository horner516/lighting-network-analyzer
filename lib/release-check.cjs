...
function createReleaseChecker(currentVersion, request = fetch) {
  ...
  return function check() {
    ...
    pending = (async () => {
      let response;
      try {
        response = await request(releaseApi, { signal: AbortSignal.timeout(8000), redirect: 'manual', headers: { Accept: 'application/vnd.github+json', 'User-Agent': 'Lux-Link-update-check' } });
